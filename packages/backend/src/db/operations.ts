 ...
 */
export const getOrCreateUser = async (clerkUserId: string, email: string, name?: string) => {
  try {
 ...
 */
export const createGuestSession = async (sessionId: string) => {
  try {
 ...
 */
export const getOrCreateGuestSession = async (sessionId: string) => {
 ...
 */
export const canCreateProject = async (user: { id: string; type: string }) => {
  try {
    const limits = LIMITS[user.type as keyof typeof LIMITS];
 ...
 */
export const canSendChat = async (user: { type: string }, projectId: string) => {
  try {
    const limits = LIMITS[user.type as keyof typeof LIMITS];
 ...
 */
export const getRemainingLimits = async (user: { id: string; type: string }) => {
  try {
    const limits = LIMITS[user.type as keyof typeof LIMITS];
 ...
 */
export const getProject = async (user: { id: string }, projectId: string) => {
  try {
 ...
 */
export const getAllProjects = async (user: { id: string }) => {
  try {
 ...
 */
export const addChatToProject = async (
  user: { id: string },
  projectId: string,
  userMessage: string,
  aiResponse: string
 ...
 */
export const getConversationHistory = async (user: { id: string }, projectId: string) => {
  try {
    // Verify project belongs to user
    const project = await db.query.projects.findFirst({
      where: and(eq(projects.id, projectId), eq(projects.userId, user.id)),
 ...
 */
export const clearConversationHistory = async (user: { id: string }, projectId: string) => {
  try {
    // Verify project belongs to user
    const project = await db.query.projects.findFirst({
      where: and(eq(projects.id, projectId), eq(projects.userId, user.id)),
 ...
 * Delete a project
 */
export const deleteProject = async (user: { id: string }, projectId: string) => {
  try {
    const result = await db
      .delete(projects)
      .where(and(eq(projects.id, projectId), eq(projects.userId, user.id)))
...
export const getAllData = async () => {
  try {
    ...
    // Get all guest sessions with their projects
    const allGuestSessions = await db.query.guestSessions.findMany();

    // Get projects for each guest (projects where userId starts with 'guest_')
    const guestProjects = await db.query.projects.findMany({
      where: sql`${projects.userId} LIKE 'guest_%'`,
    });
    ...
    const guestsData = await Promise.all(
      allGuestSessions.map(async (guest) => {
        const guestProjectsList = guestProjects.filter((p) => p.userId === guest.id);
